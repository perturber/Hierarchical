...
class StableEMRIFisher:
    ...
    def __init__(self, M, mu, a, p0, e0, Y0, dist, qS, phiS, qK, phiK,
                 Phi_phi0, Phi_theta0, Phi_r0, Al, nl, Ag=0.0, ng=0.0, dt = 10., T = 1.0, param_args = None, EMRI_waveform_gen = None, PSD = None, window = None,
	...
	# Determine what version of TDI to use or whether to use the LWA 
        ...
        if self.traj_use_rk4:
            logger.debug("using Runge-Kutta 4 integrator.")
        ...
        elif 'Disk_Relativistic_Kerr_Circ_Flux' in self.traj_module_func:
            self.waveform_model_choice = "Disk_RelativisticKerrCircularFlux"
        elif 'Joint_Relativistic_Kerr_Circ_Flux' in self.traj_module_func:
            self.waveform_model_choice = "Joint_RelativisticKerrCircularFlux"
        ...
        self.traj_params.update({'A_l':Al,
                      'n_l':nl,
                      'A_g':Ag,
                      'n_g':ng,
                      }
    ...
    def SNRcalc_SEF(self):
        ...
        # Compute SNR
        logger.debug(f"Computing SNR for parameters: {self.wave_params}") 
    ...
    def check_if_plunging(self):
        ...
        if t_traj[-1] < self.T*YRSID_SI:
            ...
            logger.debug("Body is not plunging, Fisher should be stable.")
    ...
    def Fisher_Stability(self):
        ...
        for i in range(len(self.param_names)):
            ...
            if self.wave_params[self.param_names[i]] == 0.0:
                if self.param_names[i] == 'A_l':
                    delta_init = np.geomspace(1e-7,1e-9,Ndelta)
                elif self.param_names[i] == 'A_g':
                    delta_init = np.geomspace(1e-11,1e-14,Ndelta)
                else:
            ...
            # Compute Ndelta number of delta values to compute derivative. Testing stability.
                ...
                delta_init = np.geomspace(1e-4*self.wave_params[self.param_names[i]],1e-9*self.wave_params[self.param_names[i]],Ndelta)
            elif self.param_names[i] == 'A_l':
                delta_init = np.geomspace(1e-1*self.wave_params[self.param_names[i]],max(1e-9,1e-4*self.wave_params[self.param_names[i]]),Ndelta)
            else:
                delta_init = np.geomspace(1e-1*self.wave_params[self.param_names[i]],max(1e-14,1e-10*self.wave_params[self.param_names[i]]),Ndelta)
